...
if _proxy_block_cooldown_seconds_env is not None:
    PROXY_BLOCK_COOLDOWN_SECONDS = max(1, int(_proxy_block_cooldown_seconds_env))
elif _proxy_block_cooldown_minutes_env is not None:
    PROXY_BLOCK_COOLDOWN_SECONDS = max(1, int(_proxy_block_cooldown_minutes_env) * 60)
else:
